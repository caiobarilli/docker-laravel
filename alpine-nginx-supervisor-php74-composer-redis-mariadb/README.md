# Alpine - Nginx - Php - Composer - Supervisor - Redis - MariaDb
...
**MariaDb**: Banco de dados relacional MySQL para armazenar dados do aplicativo.
...
- `lumen:install`: Instala um novo projeto Lumen dentro do contêiner Docker.
- `laravel:permissions`: Configura as permissões do projeto Laravel dentro do contêiner Docker.
- `lumen:permissions`: Configura as permissões do projeto Lumen dentro do contêiner Docker.
...
./run lumen:install   # Instala um novo projeto Lumen dentro do contêiner Docker.
./run laravel:permissions  # Configura as permissões do projeto Laravel dentro do contêiner Docker.
./run lumen:permissions    # Configura as permissões do projeto Lumen dentro do contêiner Docker.
```
